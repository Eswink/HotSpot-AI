...
function hotspot_ai_add_settings_link($links)
{
    ...
    $settings_link = '<a href="admin.php?page=hotspot">' . __('设置', 'hotspot-ai') . '</a>';
...
function hotspot_ai_activate_redirect()
{
    if (get_option('hotspot_ai_do_activation_redirect', false)) {
        ...
        wp_safe_redirect(admin_url('admin.php?page=hotspot'));
        exit;
    }
}
...
{
    $response = wp_remote_get(rest_url('hotspot/v1'));
